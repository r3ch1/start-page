          ...
          (dt: string, i: any) =>
            i <= 3 && (
              <div className="col" key={`weather-${i}`}>
                ...
                <img
                  src={`${process.env.REACT_APP_WEATHER_ICON_URL}/${weatherData.list[dt].weather.list[weatherData.list[dt].weather.list.length / 2].icon}.png`.replace(
                    'n.png',
                    'd.png',
                  )}
                  alt={weatherData.list[dt].weather.list[weatherData.list[dt].weather.list.length / 2].description}
                />
                <p className="text-small m-0">
                  {weatherData.list[dt].weather.list[weatherData.list[dt].weather.list.length / 2].description}
                </p>
